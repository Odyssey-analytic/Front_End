  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error.response && error.response.data) {
        ...
        if (serverMessage === 'Invalid password') setErrorMessage('رمز عبور معتبر نمی‌باشد.');
        else if (serverMessage === 'Invalid username/email') setErrorMessage('نام کاربری یا ایمیل موجود نیست.');
  ...
  return (
    <div className={styles.loginPageContainer}>
      <div className={`d-flex align-items-center position-fixed top-0 end-0 ms-4 mt-4`}>
        <div className={`${styles.loginBrandText} text-white me-3`}>ODESSAY</div>
        <img src={OdessayLogo} alt="Odessay Logo" className={`${styles.loginLogoImg} me-4`} />
      </div>

      {isLoading && (
        <div className={styles.loginLoadingOverlay}><div className={styles.loginSpinner}></div></div>
      )}

      <div className={styles.loginBox}>
        <Link to="/" className={`text-muted d-block text-end ${styles.loginBackToHome}`}>← بازگشت به صفحه اصلی</Link>
        <h2 className={`fw-bold mb-3 ${styles.loginTitle}`}>ورود</h2>
        <p className={`text-muted ${styles.loginSubtitle}`}>ورود با استفاده از ایمیل</p>

        <form onSubmit={handleLogin}>
          <div className={`mb-3 position-relative ${styles.loginInputWrapper}`}>
            <input type="text" className={`form-control ${styles.loginInput}`} placeholder="ایمیل یا نام کاربری خود را وارد کنید." value={email} onChange={handleEmailChange} />
            <img src={login_email_icon} alt="email icon" className={styles.loginEmailIcon} />
            {emailError && (
          ...
          <div className={`mb-3 position-relative ${styles.loginInputWrapper}`}>
            <input type={showPassword ? 'text' : 'password'} className={`form-control ${styles.loginInput}`} placeholder="رمز عبور خود را وارد کنید." value={password} onChange={handlePasswordChange} />
            <img src={login_padlock_icon} alt="password icon" className={styles.loginPasswordIcon} />
            <img src={showPassword ? login_eye_icon : login_eye_off_icon} alt="toggle password" className={styles.loginEyeIcon} onClick={() => setShowPassword(prev => !prev)} />
            {passwordError && (
              <div className={styles.loginInputErrorPopup} key={passwordErrorKey}>
          ...
          <button type="submit" className={`btn ${styles.loginBtn}`}>ورود</button>

          <div className={`text-muted small px-2 my-3 ${styles.loginNonRegisterRememberPassword}`}>
            <span>
              ثبت‌نام نکرده‌اید؟{' '}
              <Link to="/signup" className={`fw-bold text-decoration-none ${styles.loginNonRegisterRememberPassword}`}>
                ایجاد حساب
              </Link>
            </span>
            <div>
              <Link to="/forgot-password" className={`fw-bold text-decoration-none ${styles.loginNonRegisterRememberPassword}`}>
                فراموشی رمز عبور
              </Link>
            </div>
          </div>
          ...
          {loginStatus && (
            <div className={`${styles.loginPopupWarningOverlay} ${styles[loginStatus]}`}>
              <div className={styles.loginWarningPopupCard}>
                <button className={styles.loginWarningPopupCloseBtn} onClick={() => setLoginStatus('')}>×</button>
                <img src={loginStatus === 'success' ? successful_signup_icon : unsuccessful_signup_icon} className={styles.loginWarningPopupEmoji} alt="status-icon" />
                <div className={styles.loginWarningPopupLineSeparator}></div>
                <h5>{loginStatus === 'success' ? 'ورود با موفقیت انجام شد!' : 'خطایی رخ داده است!'}</h5>
                <p className="text-muted small">{loginStatus === 'success' ? 'در حال انتقال به حساب کاربری...' : errorMessage}</p>
              </div>
            </div>
          )}
