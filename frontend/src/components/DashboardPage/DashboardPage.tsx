...
const DashboardPage = () => {
  ...
  useEffect(() => {
    // ❗ وقتی بک آماده شد، این کامنت رو بردار:
    /*
    const getGames = async () => {
      try {
        const res = await fetchUserGames();
        setGames(res);
      } catch (err) {
...
// تابع fetchUserGames رو توی userService.ts بنویس (همونی که قبلاً توضیح دادم).

// اون قسمت کامنت‌شده توی useEffect رو آنکامنت کن.

// setGames(mockGames); رو حذف کن.