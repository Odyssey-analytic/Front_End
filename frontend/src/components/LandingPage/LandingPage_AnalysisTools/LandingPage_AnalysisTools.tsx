...
const LandingPage_AnalysisTools = () => {
  ...
  // one-shot guards
  const chartPlayedRef = useRef(false);
  const rightPlayedRef  = useRef(false);

  /* ========== Local State (final states remain fixed) ========== */
  ...
  /* ========== Intersection Observer Logic (one-shot) ========== */
  useEffect(() => {
    ...
    green.style.strokeDasharray = String(greenLength);
    gray.style.strokeDasharray  = String(grayLength);

    const chartObserver = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          if (!entry.isIntersecting || chartPlayedRef.current) return;

          // initial reset just before first run
          green.style.strokeDashoffset = String(greenLength);
          gray.style.strokeDashoffset  = String(grayLength);
          ...
          const t2 = setTimeout(() => setShowZero(true), 1400);
          ...
          const t4 = setTimeout(() => setShowCenterText(true), 2300);
          ...
          chartPlayedRef.current = true;
          chartObserver.unobserve(entry.target); // prevent re-run
    ...
    const rightObserver = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          if (!entry.isIntersecting || rightPlayedRef.current) return;
          setAnimateRight(true);          // trigger once
          rightPlayedRef.current = true;
  ...
  return (
    <div className={styles.analysisToolsWrapper}>
      ...
      <div className={styles.imageContainer}>
        ...
        {/* Chart labels/texts (set once via state; no re-run) */}
        {showTitle && (
          <motion.h3
