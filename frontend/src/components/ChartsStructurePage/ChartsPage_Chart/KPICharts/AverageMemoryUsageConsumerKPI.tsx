...
const AverageMemoryUsageConsumer = ({selectedTime}) => {
  ...
  // );
    sseRef.current = new EventSource(
    `https://odysseyanalytics.ir/api/kpi/sse/AverageMemoryUsage?product_id=${gameId}&start_time=${selectedTime}&update_interval=${interval}`
  );
...
sseRef.current.onmessage = (event: MessageEvent) => {
  ...
  return (
    <ChartCardWrapper title="AverageMemoryUsageConsumer">
...
export default AverageMemoryUsageConsumer;
