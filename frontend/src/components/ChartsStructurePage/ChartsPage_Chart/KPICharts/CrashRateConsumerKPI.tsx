...
const CrashRateConsumer = ({selectedTime}) => {
  ...
  // );
    sseRef.current = new EventSource(
    `https://odysseyanalytics.ir/api/kpi/sse/CrashRate?product_id=${gameId}&start_time=${selectedTime}&update_interval=${interval}`
  );
...
sseRef.current.onmessage = (event: MessageEvent) => {
  ...
  return (
    <ChartCardWrapper title="CrashRateConsumer">
...
export default CrashRateConsumer;
