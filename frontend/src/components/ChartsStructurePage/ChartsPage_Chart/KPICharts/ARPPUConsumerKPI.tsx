...
const ARPPUConsumer = ({selectedTime}) => {
  ...
  // );
    sseRef.current = new EventSource(
    `https://odysseyanalytics.ir/api/kpi/sse/ARPPU?product_id=${gameId}&start_time=${selectedTime}&update_interval=${interval}`
  );
...
sseRef.current.onmessage = (event: MessageEvent) => {
  ...
  return (
    <ChartCardWrapper title="ARPPUConsumer">
